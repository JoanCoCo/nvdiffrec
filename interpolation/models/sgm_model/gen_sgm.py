...
from interpolation.models.sgm_model.linefiller.thinning import thinning
from interpolation.models.sgm_model.linefiller.trappedball_fill import trapped_ball_fill_multi, flood_fill_multi, mark_fill, build_fill_map, merge_fill, \
...
from interpolation.models.sgm_model import softsplat
from interpolation.models.sgm_model.forward_warp2 import ForwardWarp
from interpolation.models.sgm_model.my_models import create_VGGFeatNet
from interpolation.models.sgm_model.vis_flow import flow_to_color
...
def get_deformable_flow(flowObj, img1, mask_1, box_1, 
    ...
    if warp_func is not None:
        patch_flow_tensor = torch.Tensor(patch_flow.transpose([2,0,1])).unsqueeze(0).contiguous()
        fwarp_mask_tensor = torch.Tensor(mask1_patch_pad).unsqueeze(0).unsqueeze(0).contiguous()
        if use_gpu:
...
def get_guidance_flow(label_map1, label_map2, img1, img2,
                        ...
                        rank_sum_thr=0, use_gpu=False, maxPixNum=0):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--height', default=550,
                    help='height of the generated flow, default: 960')
    parser.add_argument('--width', default=550,
    ...
    for f_idx, folder in enumerate(folderList[start_idx:end_idx]):
        ...
        guideflow_1to3, matching_color_1to3 = get_guidance_flow(labelMap1, labelMap3, img1_rs, img3_rs,
                                ...
                                rank_sum_thr = rankSumThr, use_gpu = use_gpu, maxPixNum=maxPixNum)
