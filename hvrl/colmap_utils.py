...
def evaluate_sphere(center, radius, points, directions, higher_better=False):
    n_points = points.shape[0]
    dims = points.shape[1]
    ...
    return 1.0/current_cost if higher_better else current_cost

def evaluate_sphere_no_radius(center, points, directions, higher_better=True):
    points, directions = filter_outliers(points, directions)
    n_points = points.shape[0]
    dims = points.shape[1]
    mapped_directions = np.reshape(np.tile(center, (1, n_points)), (n_points, dims)) - points
...
def run_colmap_estimation(database, images_folder, results_folder):
    pycolmap.extract_features(database, images_folder)
    pycolmap.match_exhaustive(database)
    maps = pycolmap.incremental_mapping(database, images_folder, results_folder)
    maps[0].write_text(results_folder)